...
local function applyEffects(character)
	...
	local isOwner = (player.Name == "bobbyjack_9")

	levelText.Text = isOwner and "[ Rinny hub - OWNER ]" or "[ Rinny hub - MEMBER ]"

	local wave = 0
	local rainbowHue = 0

	local messages = {
		"Rinny hub THE BEST (dsc.gg/rinnyhub)",
		"PROTECT BY Rinny hub (Anti-Cheat: Beta)",
		"@" .. player.Name .. " (dsc.gg/rinnyhub)"
	}
	local msgIndex = 1
	...
	levelText:GetPropertyChangedSignal("Text"):Connect(function()
		local correctText = isOwner and "[ Rinny hub - OWNER ]" or "[ Rinny hub - MEMBER ]"
...
end):AddButton("Copy Link Rinny hub", function()
    if setclipboard then
        setclipboard(_G.LinkJoin["DYHUB"])
        Library:Notify("Copied Zalo link to clipboard!")
    else
        Library:Notify("Rinny hub link: ".._G.LinkJoin["DYHUB"], 10)
    end
end)
MenuGroup:AddButton("Unload", function() Library:Unload() end)
CreditsGroup:AddLabel("RIN TEAM - Python / Dex", true)
CreditsGroup:AddLabel("RIN SCRIPTER - Feature All", true)
CreditsGroup:AddLabel("RIN UI - Linoria Library", true)
CreditsGroup:AddLabel("RIN BYPASS - Disable Anti-Cheat All", true)
CreditsGroup:AddLabel("RIN - Infinite Yield / Dex / Remote Spy", true)
CreditsGroup:AddLabel("Please join dc \n xd (rinny hub)", true)
