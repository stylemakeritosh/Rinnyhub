...
local Window = Library:Window({
    Title = "Rinny Hub | The Strongest Battleground",
    Desc = "Credits: rintoshiii",
    Icon = 109251560,
    Theme = "Dark",
    Config = {
...
local Misc = Window:Tab({Title = "Misc", Icon = "settings"}) do
    Misc:Section({Title = "Misc Features"})

    Misc:Button({
        Title = "Trash Man",
        Desc = "Load Trash Man script",
        Callback = function()
    ...
    Misc:Button({
        Title = "Kyoto Combo V2",
        Desc = "Load Kyoto Combo V2",
        Callback = function()
    ...
    Misc:Button({
        Title = "Auto Block V11",
        Desc = "Load Auto Block V11",
        Callback = function()
    ...
    Misc:Button({
        Title = "Auto Down Slam",
        Desc = "Load Auto Down Slam",
        Callback = function()
    ...
    Misc:Button({
        Title = "Back Dash Cancel",
        Desc = "Load Back Dash Cancel",
        Callback = function()
    ...
    Misc:Button({
        Title = "Instant Twisted Revamp",
        Desc = "Load Instant Twisted Revamp",
        Callback = function()
    ...
    Misc:Button({
        Title = "Infinite Dash V1",
        Desc = "Load Infinite Dash V1",
        Callback = function()
    ...
    Misc:Button({
        Title = "Teleporter Tatsumaki",
        Desc = "Load Teleporter Tatsumaki",
        Callback = function()
    ...
    Misc:Button({
        Title = "No Slide Dash EndLag",
        Desc = "Load No Slide Dash EndLag",
        Callback = function()
...
-- Tab: Local
local Local = Window:Tab({Title = "Local", Icon = "user"}) do
    Local:Section({Title = "Local Features"})
...
local Esp = Window:Tab({Title = "Esp", Icon = "eye"}) do
    ...
    Esp:Toggle({
        Title = "Esp Character",
        Desc = "Toggle Esp Character",
        Value = false,
        Callback = function(Value)
            espEnabledChar = Value

            if espEnabledChar then
                refreshAll()
...
-- Tab: Dummy
local Dummy = Window:Tab({Title = "Dummy", Icon = "robot"}) do
    Dummy:Section({Title = "Dummy Features"})

    -- Highlight
    local npcHighlights = {}
    local npcConnections = {}
    local highlightEnabledDummy = false
    ...
    Dummy:Toggle({
        Title = "Highlight",
        Desc = "Toggle Dummy Highlight",
        Value = false,
        Callback = function(Value)
            highlightEnabledDummy = Value
            if Value then
                enableNPCHighlights()
            else
...
-- Tab: Tech
local Tech = Window:Tab({Title = "Tech", Icon = "code"}) do
    Tech:Section({Title = "Tech Features"})
...
-- Tab: Auto
local Auto = Window:Tab({Title = "Auto", Icon = "play"}) do
    Auto:Section({Title = "Auto Features"})
    ...
    -- Auto Supa
    local targetAnimationIdSupa = "rbxassetid://12296113986"
    local rotating = false
    local animConnectionSupa = nil
    local charConnectionSupa = nil
    ...
    local function bindHumanoid(humanoid)
        animConnectionSupa = humanoid.AnimationPlayed:Connect(function(track)
            if track.Animation and track.Animation.AnimationId == targetAnimationIdSupa and not rotating then
                rotating = true
                holdJump()
    ...
    Auto:Toggle({
        Title = "Auto Supa",
        Desc = "Toggle Auto Supa",
        Value = false,
        Callback = function(Value)
            if Value then
                setupMonitor()
                charConnectionSupa = player.CharacterAdded:Connect(setupMonitor)
            else
                if animConnectionSupa then animConnectionSupa:Disconnect() animConnectionSupa = nil end
                if charConnectionSupa then charConnectionSupa:Disconnect() charConnectionSupa = nil end
            end
        end,
    })

    -- Flowing + Grasp
    local animationIdFlow = "rbxassetid://12273188754"
    local toggleConnectionFlow = nil
    local systemEnabledFlow = false

    Auto:Toggle({
        Title = "Flowing + Grasp",
        Desc = "Toggle Flowing + Grasp",
        Value = false,
        Callback = function(Value)
            systemEnabledFlow = Value

            if toggleConnectionFlow then
                toggleConnectionFlow:Disconnect()
                toggleConnectionFlow = nil
            end

            if not systemEnabledFlow then return end

            local isTweening = false
            local lastPlaying = false

            toggleConnectionFlow = game:GetService("RunService").RenderStepped:Connect(function()
                local char = player.Character
                if not char then return end
                ...
                for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
                    if track.Animation and track.Animation.AnimationId == animationIdFlow then
                        isPlaying = true
                        break
                    end
                end
    ...
    -- Upper + Grasp
    local animationIdUpper = "rbxassetid://10503381238"
    local TWEEN_HEIGHT_OFFSET = Vector3.new(0, 8, 0)
    local systemEnabledUpper = false
    local toggleConnectionUpper = nil

    Auto:Toggle({
        Title = "Upper + Grasp",
        Desc = "Toggle Upper + Grasp",
        Value = false,
        Callback = function(Value)
            systemEnabledUpper = Value

            if toggleConnectionUpper then
                toggleConnectionUpper:Disconnect()
                toggleConnectionUpper = nil
            end

            if not systemEnabledUpper then return end

            local isTweening = false
            local lastPlaying = false
            local cooldown = false

            toggleConnectionUpper = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                local isPlaying = false
                for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
                    if track.Animation and track.Animation.AnimationId == animationIdUpper then
                        isPlaying = true
                        break
                    end
                end
        ...
        Callback = function(Value)
            ...
            local function setupForCharacter(char)
                ...
                _G.SlideM1AnimConnection = animator.AnimationPlayed:Connect(function(track)
                    ...
                    if id and targetIDs[id] and not cooldownSlide then
                        cooldownSlide = true
    ...
    Auto:Toggle({
        Title = "Auto Surf",
        Desc = "Toggle Auto Surf",
        Value = false,
        Callback = function(Value)
            local Players = game:GetService("Players")
            local TweenServiceSurf = game:GetService("TweenService")
            local RunService = game:GetService("RunService")
            local LocalPlayerSurf = Players.LocalPlayer

            local TARGET_ANIM_ID = "rbxassetid://12309835105"
            local active = Value
            local isTweening = false
            local animConnectionSurf, charConnectionSurf

            if not active then
                isTweening = false
                if charConnectionSurf then charConnectionSurf:Disconnect() charConnectionSurf = nil end
                if animConnectionSurf then animConnectionSurf:Disconnect() animConnectionSurf = nil end
                return
            end

            local function getCharacter()
                return LocalPlayerSurf.Character or LocalPlayerSurf.CharacterAdded:Wait()
            end
            ...
            local function tweenForward()
                ...
                local tween = TweenServiceSurf:Create(root, tweenInfo, {CFrame = targetCFrame})
                tween:Play()
                tween.Completed:Wait()
            end
            ...
            if LocalPlayerSurf.Character then
                getCharacter()
            end
            if charConnectionSurf then charConnectionSurf:Disconnect() end
            charConnectionSurf = LocalPlayerSurf.CharacterAdded:Connect(function()
                task.wait(1)
                if active then
                    getCharacter()
                end
    ...
    -- Whirlwind Dunk
    Auto:Toggle({
        Title = "Whirlwind Dunk",
        Desc = "Toggle Whirlwind Dunk",
        Value = false,
        Callback = function(Value)
            local Players = game:GetService("Players")
            local RunService = game:GetService("RunService")
...
-- Tab: Server
local Server = Window:Tab({Title = "Server", Icon = "server"}) do
    Server:Section({Title = "Server Features"})
    ...
    Server:Button({
        Title = "Small Server",
        Desc = "Hop to small server (<5 players)",
        Callback = function()
            local TeleportService = game:GetService("TeleportService")
            local HttpService = game:GetService("HttpService")
            local player = game.Players.LocalPlayer
...
-- Tab: Auto Block
local AutoBlock = Window:Tab({Title = "Auto Block", Icon = "shield"}) do
    AutoBlock:Section({Title = "Auto Block Features"})

    AutoBlock:Button({
        Title = "Notpaki Block",
        Desc = "Load Notpaki Block",
        Callback = function()
...
-- Tab: Admin
local Admin = Window:Tab({Title = "Admin", Icon = "crown"}) do
    Admin:Section({Title = "Admin Features"})
...
Window:Notify({
    Title = "Rinny Hub",
    Desc = "Loaded successfully! Credits: rintoshiii",
    Time = 4
})